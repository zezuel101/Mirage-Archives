...
{
	/// <summary>
	/// Decodes Terrarium terrain-RGB tiles to meters, and rejects the ones that carry no data. WebIngest §4.5.
	///
	/// The encoding is <c>R·256 + G + B/256 − 32768</c> meters — see <see cref="F:Mirage.WebIngest.ImageryProvider.TerrariumDem" />
	...
	/// flattest real thing here — is never flat to the millimeter across 65k texels. Note this is a different
	/// test from the flat-color check §12 explicitly forbids for imagery: EOX's deep-ocean tiles are
	...
	{
		/// <summary>Decode a Terrarium PNG to meters, row-major, one float per texel.
		...
		// Token: 0x0600007C RID: 124 RVA: 0x00004D24 File Offset: 0x00002F24
		public static bool TryDecode(byte[] png, out float[] meters, out int width, out int height)
		{
			meters = null;
			width = (height = 0);
			byte[] rgb;
			...
			else
			{
				...
				meters = new float[count];
				result = TerrariumElevation.Fill(rgb, meters, count);
			}
			return result;
		}
		...
		/// </summary>
		// Token: 0x0600007D RID: 125 RVA: 0x00004D74 File Offset: 0x00002F74
		...
		// Token: 0x0600007E RID: 126 RVA: 0x00004E20 File Offset: 0x00003020
		...
		// Token: 0x06000080 RID: 128 RVA: 0x00004EE4 File Offset: 0x000030E4
		public static bool IsNoData(float[] meters)
		{
			bool flag = meters == null || meters.Length == 0;
			bool result;
			if (flag)
			{
				result = true;
		...
		/// <summary>Mean radius of the planet this DEM actually measures. Terrarium is Earth, in real meters,
