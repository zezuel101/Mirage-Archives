...
namespace Mirage.WebIngest
{
	...
	public sealed class ImageryProvider
	{
		...
		// (get) Token: 0x06000085 RID: 133 RVA: 0x0000551E File Offset: 0x0000371E
		public string Name { get; }

		/// <summary>URL template — {0} = z, {1} = y, {2} = x. Note the y/x ordering, matching WMTS row/col.</summary>
		// Token: 0x1700001A RID: 26
		// (get) Token: 0x06000086 RID: 134 RVA: 0x00005526 File Offset: 0x00003726
		...
		// (get) Token: 0x06000087 RID: 135 RVA: 0x0000552E File Offset: 0x0000372E
		...
		// (get) Token: 0x06000088 RID: 136 RVA: 0x00005536 File Offset: 0x00003736
		...
		// (get) Token: 0x06000089 RID: 137 RVA: 0x0000553E File Offset: 0x0000373E
		...
		// (get) Token: 0x0600008A RID: 138 RVA: 0x00005546 File Offset: 0x00003746
		public TileImageFormat Format { get; }

		// Token: 0x0600008B RID: 139 RVA: 0x0000554E File Offset: 0x0000374E
		...
		// Token: 0x0600008C RID: 140 RVA: 0x00005585 File Offset: 0x00003785
		...
		// (get) Token: 0x0600008D RID: 141 RVA: 0x000055A4 File Offset: 0x000037A4
		...
		/// with uniform color processing — no patchwork anywhere — at the cost of a hard ~10 m/px ceiling (z14).
		...
		/// <summary>
		/// NASA GIBS MODIS Terra true color. U.S. federal government work — public domain, no usage restriction
		...
		///
		/// Not the default: MODIS true color is a raw DAILY snapshot, not a cloud-filtered composite, so any
		/// given area can easily be mostly cloud. EOX's "cloudless" is a genuine multi-date composite.
		/// </summary>
		// Token: 0x0400006E RID: 110
		public static readonly ImageryProvider GibsModis = new ImageryProvider("gibs_modis", "https://gibs.earthdata.nasa.gov/wmts/epsg3857/best/MODIS_Terra_CorrectedReflectance_TrueColor/default/" + ImageryProvider.GibsYesterday + "/GoogleMapsCompatible_Level9/{0}/{1}/{2}.jpeg", 9, "MODIS Terra true color courtesy of NASA EOSDIS GIBS", false, TileImageFormat.Jpeg);
		...
		/// ocean bathymetry, served to z15. Elevation is packed as <c>R·256 + G + B/256 − 32768</c> meters.
		///
		/// <b>PNG, not JPEG, and that is not a detail.</b> The encoding puts one meter in the G channel's LSB,
		/// so lossy compression is catastrophic here: a single unit of chroma error is a 25 cm step, and JPEG's
		/// 4:2:0 chroma subsampling would smear elevation across neighboring texels. Hence
		...
		/// and a tile is resident only when EVERY present layer landed. Baking color past canonical's finest
		/// height level would fail the whole tile group, so "stream color from the web" is not actually an
		...
		/// <summary>Color imagery sources (the ones a user picks between).</summary>
