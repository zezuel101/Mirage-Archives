...
namespace Mirage.WebIngest
{
	...
	public static class MirageCubeMath
	{
		...
		// (get) Token: 0x060000D5 RID: 213 RVA: 0x00007FCC File Offset: 0x000061CC
		// (set) Token: 0x060000D6 RID: 214 RVA: 0x00007FD3 File Offset: 0x000061D3
		...
		// Token: 0x060000D7 RID: 215 RVA: 0x00007FDC File Offset: 0x000061DC
		...
		/// [-1, 1], then normalize.
		/// </summary>
		// Token: 0x060000D8 RID: 216 RVA: 0x00008040 File Offset: 0x00006240
		public static void FaceUVToDirection(int face, double u, double v, out double x, out double y, out double z)
		{
			...
			if (flag)
			{
				throw new InvalidOperationException("MirageCubeMath: face basis not pinned — run TestBench --test-cube to measure it.");
			}
			double s = 2.0 * u - 1.0;
			double t = 2.0 * v - 1.0;
		...
		/// <c>(π/2·R)/2^L</c> — normalised so the face CENTER is exactly 1.0.
		///
		...
		/// the face center covers 2x the ground of one at a face edge, and 2.1x one at a corner. Anything that
		/// converts between "a level" and "meters on the ground" and treats a level as one fixed size is wrong
		...
		/// area-equivalent scalar), divided by the face-center value of 1 to normalise. Verified against tile
		...
		/// </summary>
		// Token: 0x060000DA RID: 218 RVA: 0x00008240 File Offset: 0x00006440
		...
		/// </summary>
		// Token: 0x060000DB RID: 219 RVA: 0x000082B8 File Offset: 0x000064B8
		...
		// Token: 0x060000DC RID: 220 RVA: 0x00008368 File Offset: 0x00006568
		...
		// Token: 0x060000DD RID: 221 RVA: 0x000083B8 File Offset: 0x000065B8
		...
		/// </summary>
		// Token: 0x060000DE RID: 222 RVA: 0x00008404 File Offset: 0x00006604
		...
		/// </summary>
		// Token: 0x060000DF RID: 223 RVA: 0x00008480 File Offset: 0x00006680
		...
		/// border: slot = tileSize + 2·borderPx, and the border over-fetches neighboring imagery so filtering
		/// across a tile edge stays seamless (§9).
		///
		/// <b>Texel indices are sample POINTS, not cell centers — there is deliberately no +0.5 here.</b> This
		...
		/// <c>floor(px)</c>; at fractional 0 the MN weights collapse to a blend centered exactly on
		...
		///
		/// Writing tiles on a cell-centered grid (with the +0.5 this originally had) while the sampler reads them
		...
		/// </summary>
		/// <summary>
		/// <see cref="M:Mirage.WebIngest.MirageCubeMath.TileTexelToLatLon(System.Int32,System.Int32,System.Int32,System.Int32,System.Double,System.Double,System.Int32,System.Int32,System.Double@,System.Double@)" /> stopped one step earlier: the unit DIRECTION in the body frame.
		///
		/// Exists because the streamer's per-tile hot path only ever wanted the direction. Going through lat/lon
		/// costs an <c>asin</c> and an <c>atan2</c> to leave the direction, then two <c>cos</c> and two
		/// <c>sin</c> to rebuild it — six transcendentals per tile, thousands of tiles per frame, to compute the
		/// vector it already had. Same chain, same result, minus the round trip; <see cref="M:Mirage.WebIngest.MirageCubeMath.TileTexelToLatLon(System.Int32,System.Int32,System.Int32,System.Int32,System.Double,System.Double,System.Int32,System.Int32,System.Double@,System.Double@)" />
		/// now forwards here so the two cannot drift.
		/// </summary>
		// Token: 0x060000E0 RID: 224 RVA: 0x000084FC File Offset: 0x000066FC
		public static void TileTexelToDirection(int face, int level, int tx, int ty, double px, double py, int tileSize, int borderPx, out double x, out double y, out double z)
		{
			int grid = 1 << level;
		...
		// Token: 0x060000E1 RID: 225 RVA: 0x00008560 File Offset: 0x00006760
		public static void TileTexelToLatLon(int face, int level, int tx, int ty, double px, double py, int tileSize, int borderPx, out double lat, out double lon)
		{
			...
			double y;
			double z;
			MirageCubeMath.TileTexelToDirection(face, level, tx, ty, px, py, tileSize, borderPx, out x, out y, out z);
			MirageCubeMath.DirectionToLatLon(x, y, z, out lat, out lon);
		}

		// Token: 0x060000E2 RID: 226 RVA: 0x00008594 File Offset: 0x00006794
		...
		/// </summary>
		/// <remarks>
		/// MEASURED by `TestBench --test-cube` against Sol's shipped Earth height archive, correlated
