...
namespace Mirage.WebIngest
{
	...
	/// atlases, resident only when every present layer landed), so a color-only bake past canonical's finest
	...
	///
	/// <b>Ordering: DEM first, color last.</b> The DEM decides everything — it is needed by two layers, it is
	...
	public sealed class CubeTileBaker : ITileBaker
	{
		...
		// (get) Token: 0x0600006E RID: 110 RVA: 0x0000430A File Offset: 0x0000250A
		private int Slot
		{
			get
			{
		...
		// Token: 0x0600006F RID: 111 RVA: 0x0000431C File Offset: 0x0000251C
		...
		// Token: 0x06000071 RID: 113 RVA: 0x000044C4 File Offset: 0x000026C4
		[DebuggerStepThrough]
		...
		// Token: 0x06000073 RID: 115 RVA: 0x000045AC File Offset: 0x000027AC
		[DebuggerStepThrough]
		...
		///
		/// Keyed off <c>meters == 0</c>: Terrarium writes exactly 0 for ocean, and the MN reproject leaves the
		...
		[DebuggerStepThrough]
		private Task<IngestOutcome> FillBathymetry(float[] meters, int face, int level, int tx, int ty, CancellationToken ct)
		{
			...
			<FillBathymetry>d__.meters = meters;
		...
		/// neighbors at the tile edge). Returns false if the box wraps the antimeridian — a single GridServer
		...
		// Token: 0x06000075 RID: 117 RVA: 0x000046A4 File Offset: 0x000028A4
		[DebuggerStepThrough]
		...
		private bool BakeNormal(BakedTile result, MercatorGather demGather, int demZoom, int face, int level, int tx, int ty, BakeBuffers buf)
		{
			...
			if (flag)
			{
				MirageDebug.LogError(string.Format("CubeTileBaker: normal stencil left the gather for L{0} f{1} {2},{3} — the ", new object[]
				{
					...
					tx,
					ty
				}) + "RequiredTiles neighbor expansion is too tight for the stencil's reach.");
				result2 = false;
			}
			else
			{
		...
		// Token: 0x06000078 RID: 120 RVA: 0x000049B0 File Offset: 0x00002BB0
		[DebuggerStepThrough]
		...
		// Token: 0x06000079 RID: 121 RVA: 0x00004A34 File Offset: 0x00002C34
		...
		/// tile finer than 10 m, blocky) 0→255 step. That step lives in the color tile's ALPHA, which BC7 packs
		/// into the same block as the SMOOTH RGB imagery. A razor alpha edge that does not track the color coast
		/// is not collinear with RGB, so the block's shared interpolation line fits neither and the compressor
		/// emits the blocky color artefacts seen along coastlines — even though the source imagery is clean. A
		/// few-texel blur makes alpha a gentle ramp roughly co-located with the color transition, which both
		/// compresses cleanly (the encoder can then keep mode 6's 4-bit color) and reads better as PQS
		...
		/// slot already carries a border, so the visible tile's ramp is fed real neighbors).
		/// </summary>
		// Token: 0x0600007A RID: 122 RVA: 0x00004A68 File Offset: 0x00002C68
		...
		/// <summary>
		/// Multiplier from the DEM's real-world meters to THIS body's meters. 1.0 for a real-scale Earth; 0.25
		/// for a quarter-scale one.
		///
		/// <b>Not cosmetic — without it the bake is garbage on any rescaled system.</b> Terrarium reports real
		/// Earth elevations (−10,919 m … +8,748 m). A quarter-scale Sol's Earth has deformity 5250 /
		/// offset −3000, i.e. an R16 range of just [−3000, +2250] m, so every real elevation above
		/// 2250 m would clamp flat — the entire Himalaya, Andes and Alps rendered as plateaux — while
		/// <see cref="T:Mirage.WebIngest.NormalFromHeight" /> combined real relief with a quarter-size radius and produced normals
		/// about 4x too steep.
		///
		/// Scaling here rather than at the quantizer is what keeps it correct in both places at once: the
		...
		/// <summary>Color post-process making the web imagery match the canonical look (RGB only). Identity when
		/// grading is off.</summary>
		// Token: 0x0400005B RID: 91
		private readonly ColorGrade colorGrade;

		/// <summary>Write the color tile's alpha as a water mask (white = water) derived from the height layer,
		...
		/// before it enters color-alpha and BC7. 0 = no blur (hard edge). See <see cref="M:Mirage.WebIngest.CubeTileBaker.BlurWaterMask(System.Byte[],System.Int32,System.Single,Mirage.WebIngest.BakeBuffers)" />.</summary>
		// Token: 0x0400005D RID: 93
		private readonly float waterMaskBlurPx;

		/// <summary>Sea level in the DEM's meters (Terrarium/GMRT are absolute, 0 = sea level; the body rescale is
		/// linear so 0 maps to 0). At or below this a texel is water.</summary>
		// Token: 0x0400005E RID: 94
		private const float SeaLevelMeters = 0f;
