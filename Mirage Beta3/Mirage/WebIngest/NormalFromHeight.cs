...
namespace Mirage.WebIngest
{
	...
	/// The frame is east/north/radial (below). The stencil is the neighbor in +longitude and the neighbor in
	/// +latitude — NOT the neighboring cube texels. That is not a stylistic choice:
	...
	/// (NormalMapV2.py) builds the identical frame: its worldTangent points at the +longitude neighbor (east)
	...
	public static class NormalFromHeight
	{
		...
		/// </summary>
		// Token: 0x0600005E RID: 94 RVA: 0x00003500 File Offset: 0x00001700
		...
		// Token: 0x0600005F RID: 95 RVA: 0x000035C8 File Offset: 0x000017C8
		...
		// Token: 0x06000060 RID: 96 RVA: 0x00003810 File Offset: 0x00001A10
		private static bool Vertex(NormalFromHeight.ElevationSampler sample, double lat, double lon, double radius, out double x, out double y, out double z)
		{
			x = (y = (z = 0.0));
			double meters;
			bool flag = !sample(lat, lon, out meters);
			bool result;
			if (flag)
			{
				result = false;
		...
		// Token: 0x06000061 RID: 97 RVA: 0x0000387D File Offset: 0x00001A7D
		...
		/// <summary>Samples elevation in METERS at a lat/lon (degrees). Returns false where the source has no
		/// data — a normal must never be invented from a guessed elevation.</summary>
		// Token: 0x02000092 RID: 146
		// (Invoke) Token: 0x06000416 RID: 1046
		public delegate bool ElevationSampler(double lat, double lon, out double meters);
	}
}
