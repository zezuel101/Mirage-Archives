...
{
	/// <summary>
	/// A speed-optimised BC7 encoder, mode 6, <b>color-priority</b>. WebIngest P3.
	///
	...
	/// layout, branch-light. It also has the best color capability of any BC7 mode (4-bit indices, 8-bit-effective
	/// endpoints), which is why the alpha modes (5/7, ≤2-bit color) are not worth it HERE: see color-priority.
	///
	/// <b>The color tile's alpha is a secondary mask.</b> Alpha carries a <i>water mask</i> (white = water) that
	...
	///
	/// <b>Alpha endpoint ordering (keeps the mask right).</b> The bounding box fixes the color endpoints
	/// (index 0 = color-min), but alpha can run either way along that same axis. At a coast the mask
	/// (255 water → 0 land) is ANTI-correlated with color (dark → bright), so the axis-aligned box would pair
	/// color-min with alpha-min and INVERT the mask. The encoder tries both alpha orderings and keeps the lower-
	/// error one, which pairs color-min with alpha-max — so the ordinary anti-correlated coast reconstructs BOTH
	/// color and mask correctly. (One extra index pass, only on non-uniform-alpha blocks.)
	///
	/// <b>Color-priority (keeps the color right where the mask can't co-fit).</b> Where the mask is coarse
	...
	/// and the color half is the blocky coast users saw. So alpha error is down-weighted far below color in every
	/// decision (the p-bit and each index), making it a near-pure tie-breaker: color stays crisp on the line and
	...
	/// correctness is verified against an INDEPENDENT decoder (BCnEncoder.NET) by `TestBench --test-bc7`,
	/// including a misaligned-coast case that proves color survives a mask that disagrees with it. That makes
	...
	public static class Bc7Encoder
	{
		...
		/// </summary>
		// Token: 0x06000064 RID: 100 RVA: 0x00003950 File Offset: 0x00001B50
		...
		/// color-priority (see <see cref="F:Mirage.WebIngest.Bc7Encoder.AlphaErrShift" />).</summary>
		...
		// Token: 0x06000066 RID: 102 RVA: 0x00003EF7 File Offset: 0x000020F7
		...
		/// <summary>
		/// Quantize one endpoint's RGBA to 7 bits per channel plus ONE shared p-bit. Both p-bit values are tried
		/// and the one with lower COLOR-priority error wins (RGB full, alpha down-weighted — see
		...
		/// </summary>
		// Token: 0x06000067 RID: 103 RVA: 0x00003F08 File Offset: 0x00002108
		...
		// Token: 0x06000069 RID: 105 RVA: 0x00004024 File Offset: 0x00002224
		...
		/// <summary>
		/// Alpha error is right-shifted by this before it competes with color in every mode-6 decision (the
		/// shared p-bit, and each texel's index), making alpha a near-pure <b>tie-breaker</b>: it steers the index
		/// only where color barely cares (a flat block), and never overrides a real color difference.
		///
		/// Why so aggressive. The color tile's alpha is a secondary water mask / PQS-smoothness channel, never a
		/// visible one (PQS samples the RGB directly). At a coast, alpha (255→0) runs OPPOSITE to color
		...
		/// (large) error can still exceed the color cost of snapping to an alpha-matching index, so color is
		/// STILL sacrificed and the block goes blocky. A shift of 6 (alpha counts 1/64) means alpha changes the
		/// index only when the color alternatives are within a hair of each other — so color is preserved at
		/// every coast, the invisible mask absorbs the miss, and a conflict-free binary mask (flat color) still
		...
		{
			// Token: 0x0600041E RID: 1054 RVA: 0x0001CD10 File Offset: 0x0001AF10
			...
			// Token: 0x0600041F RID: 1055 RVA: 0x0001CD4C File Offset: 0x0001AF4C
