...
namespace Mirage.WebIngest
{
	...
	/// surface oscillates a couple of meters either side of sea level; any terrain sharing that band flickers
	...
	///
	/// <para>Operates in the DEM's body (game) meters — the space Scatterer's waves live in — so the band is set
	/// directly in game meters regardless of the body's elevation rescale. Identity outside <c>[min,max]</c>, so
	...
	public static class SeaLevelFlatten
	{
		...
		/// bound the band (game meters); <paramref name="slope" /> is the steepening about the midpoint (higher =
		/// thinner band residue). Heights at or beyond the band are returned unchanged.</summary>
		// Token: 0x0600005A RID: 90 RVA: 0x000032C8 File Offset: 0x000014C8
